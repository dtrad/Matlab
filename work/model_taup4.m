...
V2=V(ll+1);   
r2=r(ll+1);
Y1=real(sqrt(1-(p*V1).^2)/(r1*V1));
Y2=real(sqrt(1-(p*V2).^2)/(r2*V2));
tt=ones(NF,1);
c1=(Y1-Y2)./(Y1+Y2+eps);c1=tt*c1;
R1=c1;
...
for ll=nl-2:-1:0;
 ...
 if ll~=0 Y1=real(sqrt(1-(p*V1).^2)/(r1*V1));elseif ll==0 Y1=zeros(size(p));end
 Y2=real(sqrt(1-(p*V2).^2)/(r2*V2));
  
 tt=ones(NF,1);
 co=0;  
 c1=(Y1-Y2)./(Y1+Y2+eps);if ll==0 c1=zeros(size(Y2));end;c1=tt*c1;
...
for nlayer=1:4
   ...
   case(2)
      Ro=R1F;
   case(3)
      if nl>=4 Ro=R2F;else Ro=R0F;nlayer=4; end
   case(4)   
      Ro=R0F;
   end 
   
Ro=duplic(Ro);
Rot=ifft(Ro);%Rot=reverse(Rot);
