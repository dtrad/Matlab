% EOSC 555 --- Daniel Trad - Problem Set 1
close all
x0=2.0;
x=[0 6]
tol=1e-3;
...
[y,z] = newton2(d1fx,d2fx,x0,tol,maxiter); 

if (feval(d1fx,y)<=tol)
  if (feval(d2fx,y)<0) 
    text=sprintf('--> found a maximum at x=%5.2f with f(x)=%5.2f \n',y,feval(fx,y));
  else
    text=sprintf('--> found a minimum at x=%5.2f with f(x)=%5.2f \n',y,feval(fx,y));
  end
else
  text=sprintf('-->It did not find an extremum\n');
end
