...
%WRITESEGY WRITESEGY(filename, D, H)  writes a seismic matrix into a segy file.
%          D is the matrix of seismic traces and H the matlab structure with the 
%          trace headers.
...
%
   ...
   FID = fopen(filename,'w','b');      % when running on a SUN

%  FID = fopen(filename,'w');          % when running on a PC
