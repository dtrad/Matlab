...
wxi(1)=1;
for ix=2:nx-1
    wxi(ix)=abs(xi(ix+1)-xi(ix-1));wxi(ix)=wxi(ix)/(2*dx);
end
wxi(nx)=1;
...
for f=5:nf-30
    ...
    [MH(f,:),weights,X]=wtcgls_mwni(DH(f,:),nh,nk,nx,map,weights,niter,F);
    MHR(f,:)=idft(FR,X);
...
if (doplot>1)
    ...
    figure(ifig+5),wigb(modelr,ninterp,x,t);
    if(doplot >2) print -dpng mwni_regint.png;end
end

save model.mat model xi t;
...
function [x,w,X]=wtcgls_mwni(d,nh,nk,nx,map,w,niter,F)
    eps=1e-3;
    x=zeros(nx,1);
    X=zeros(nk,1);
    d=d(:);
