...
figure
%interpolate for better display
u3=interp(U(:,3),5);indexu=1:length(u3);
u100=interp(U(:,100),5);
subplot(311);plot(indexu,u3,indexu,u100);title('(a) singular vector u_3 and u_{100}');axis([0 128 -0.15 0.15]);xlabel('index');ylabel('amplitude')
subplot(312); picardmod(U,s,b);title('(b) clean data');axis([0 128 0.001 30]);xlabel('index');ylabel('amplitude')
subplot(313); picardmod(U,s,d);title('(c) noisy data');axis([0 128 0.001 30]);xlabel('index');ylabel('amplitude')
myprepfig
