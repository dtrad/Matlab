...
NF=512;
NH=128;
r0=1; % Surface reflection coefficient
coeff(1)=0.55;
coeff(2)=0.55;
dz(1)=100;
dz(2)=250;
...
nlayers=length(vel);
t=0:NF-1;t=t*dt;
h=-NH/4:NH/4-1;h=h*dh;
hc=-NH/2:NH/2+1;hc=hc*dh;
wav=rickerm(f,dt);
wav=padzeros(wav,NF);
%wav=[zeros(NF,NH/2-1) wav(:) zeros(NF,NH/2)];
...
for nl=nlayers:-1:1
   ...
   subplot(224),
   wigb(w1,1,hc,t);title('propagator in x-t');
   xlabel('offset')
   ylabel('time')
   ...
   W1=fft2(w1);
	...
	if nl>1
	r2=zeros(NF,NH);r2(1,NH/2)=coeff(nl-1);R2=fft2(r2);coeff2=-coeff(nl-1);
	elseif nl==1
   r2=zeros(NF,NH);r2(1,NH/2)=r0;R2=fft2(r2);coeff2=-r0;
	end;
   
   P=(W1.*(P+R1));
   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   % Multiples must be computed on x-w
   PXW=ifft(P.').';
   PMXW=PXW./(1-coeff2*PXW);
   PM=fft(PMXW.').';
   %pm=multiples(ifft(PXW));
   %PM=fft2(pm);
   %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
PM=P0;p0=ifft2(P0);clear P0;
figure
subplot(221),
wigb(p0(:,NH/4+1:3/4*NH),1,h,t);
mytext=sprintf('Primaries + Multiples'); 
title(mytext);
xlabel('offset')
ylabel('time')
...
%  P=(W1.*(P+R1).*W1);
   ...
   subplot(222+2*(nl-1)),
   amx=max(max(abs(p)));
   wigb(p(:,NH/4+1:3/4*NH),1,h,t,amx);
   ...
   if (223+2*(nl-1))<=224
   subplot(223+2*(nl-1))
   wigb(pm(:,NH/4+1:3/4*NH),1,h,t,amx);
