...
clear global
global w dte p h t np nh nt Power WV WU W dh h_near ff tor dp
...
nt=1024; % Lenght of time series
dt=0.002; % sample interval
dte=1; % Additional factor for Basis functions
t0=0;  % Initial time
nh=20; % Number of traces 
dh=20; % Offset
h_near=0; % Closest trace
axis1=[0 1 0 600];
np=20;     % Number of traces in Tau-p domain
pmax=0.9345/1000;  % Maximum p
p0=0.0499/3400; % Minimum p
CHIFACT=1;
...
NITER=15; % Maximum number of iterations for non-linear problem
...
if (optionc==3)initialiter=1;enditer=5;end; % Different sigmap to try in (3)

%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%

% Noise
randn('seed',200); % Change seed for noise 
noise=0.1*randn(nto,nh); % Noise to add to the shyntetic data
u=u+noise;  % data--->data+noise
% compute the covariance of the noise (in freq)
randn('seed',2); % Change seed for noise 
noise=0.1*randn(nto,nh);
NOISE=fft(noise);
C=cov(NOISE);
