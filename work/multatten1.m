...
load c:\daniel\thesis\P00;
load c:\daniel\thesis\P11;
[NH,NH,NF]=size(P11);
R1=1*eye(NH);
NSG=32;
NNUL=NF-100;
...
f=70;wav=rickerm(f,dt);wav=padzeros(wav,2*NF);
S=fft(wav);
coeff(1)=0.2
...
title('(a)');ylabel('offset');xlabel('time');
niter=1;
for ii=1:NF-NNUL;
   P1=P11(:,:,ii);
   P0=P00(:,:,ii);
...
figure,
subplot(121),wigb(p1(:,10:14),1,hh(10:14)-hh(NSG),t);
title('(a)');xlabel('offset');ylabel('time');
subplot(122),wigb(p(:,10:14),1,hh(10:14)-hh(NSG),t);
title('(b)');xlabel('offset');ylabel('time');
