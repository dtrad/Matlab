...
% Exercise 6. Truncated Singular Value Decomposition
% Daniel Trad
...
alfax=1;
alfay=1;
sref=0; % Multiply reference model for this scale factor
% Noise 
randn('seed',1);
stdnoise=2e-3;
...
% Introduce the data and model covariance in matrix A
AA=Wd*A*diag(1./diag(Wm));
[U,S,V]=svd(AA,0);
I=eye(length(t));
NS=((I-U*U')*t)'*((I-U*U')*t);  % Contribution to the null space to the misfit
...
for ii=1:pmax
   ...
   T=diag([ones(1,ii),zeros(1,length(mc(:))-ii)]);
   % For this example I include in T the inv(Lambda), i.e., T=T*inv(lambda)
   mc=diag(1./diag(Wm))*V*(T*diag(1./lambda))*U'*Wd*t;
   tp=A*mc;
   %if (ii==39) keyboard;end
   phim(ii)=(Wm*(mc-m0)).'*(Wm*(mc-m0));
   phid(ii)=(Wd*(t-tp)).'*(Wd*(t-tp));
   phit(ii)=phid(ii)+phim(ii);
   mytext=sprintf('p=%d\n',ii);
   if (mod(ii,10)==0) subplot(330+round(ii/10)),imagesc(mat(mc,kx));colorbar;title(mytext);end
end
figure
subplot(221);plot(phim,phid,'o'),title('L curve');
subplot(222);semilogy(phid,'o'),title('\phi_d');
subplot(223);semilogy(phim,'o'),title('\phi_m');
subplot(224);semilogy(phit,'o'),title('\phi');
...
iless=find(phid<=chiup);
%ibeta=find(yy(iless)>=chidown);

pfinal=PP(min(iless))

T=diag([ones(1,pfinal),zeros(1,length(mc(:))-pfinal)]);

mc=diag(1./diag(Wm))*V*(T*diag(1./lambda))*U'*Wd*t;
figure,imagesc(mat(mc,kx));colorbar;
tp=A*mc;
phidf=(Wd*(t-tp)).'*(Wd*(t-tp));
phif=phid(ii)+phim(ii);
phimf=(Wm*mc).'*(Wm*mc);
