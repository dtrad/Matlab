...
% ****** first determine the reverberation 1st order filter
   ...
   x=acpeo(n,lx,g1);
   if ipass==2
      subplot(223),plot(x,'b');title('output from acpeo');%hold on;
      %plot(xtad,'r'); title('x');
      gpptemp=[1;gpp(:)];rxx_gpp=autocorr(gpptemp);lgpp=length(gpptemp);
      subplot(224),plot(rxx_gpp(lgpp:-1:lgpp./2+1));title('autocorr of gpp');
      c=resta(x,-rxx_gpp(lgpp:-1:lgpp./2+1),'y');
   end;   
...
% ****** for 1st order ringing and 2nd order ringing
   ...
   % First order Filter 
   % c1.z^tau1.(1/(1+c1.z^tau1)=c1.z^tau1.(1-c1.z^tau1+c1^2.z^2tau1-....
   % Second order filter
   % -t1'c1.t1.z^(tau1+tau2).c1.z^tau1.(1/(1+c1.z^tau1)^2=...
   %                        c1.z^tau1.(1-c1.z^tau1+c1^2.z^2tau1-....
	% See Robinson and Treitel pag 288-289
   
   
   p_m=convlim(filt_1,temp1,lx,p_m); % First Order Filter
   temp1=convlim(filt_1,temp2,lx,temp1);
   temp2=convlim(filt_1,temp1,lx,temp2); % Second order filter
