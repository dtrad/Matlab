function [x,rho,eta,nit] = regradon(A,L,b_bar,k,tol,x)
...
lambda = [1e1,3,1,3e-1,1e-1,3e-2,1e-2,3e-3,1e-3,3e-4];
...
% Then compute the relative errors for the four solutions.
 ...
 %[reg_c,rho_c,eta_c] = l_corner(rho,eta,reg_param,U,s,b,'Tikh',10);
 %pause
 
 subplot(221);lambda_l = l_curve(U,s,b);   %axis([1e-3,1,1,1e3]),
 %lambda_l =input('Tikhonov hyper');
 lambda_l = 0.1;
 
 subplot(222);k_l = l_curve(U,s,b,'tsvd'); %axis([1e-3,1,1,1e3]),      
 %k_l =input('tsvd k');
 k_l =12;
 
 subplot(223);lambda_gcv = gcv(U,s,b);     %axis([1e-6,1,1e-9,1e-1]),
 %lambda_l =input('gcv hyper');
 lambda_l =0.1;
 
 subplot(224);k_gcv = gcv(U,s,b,'tsvd');   %axis([0,20,1e-9,1e-1]), 
 %k_gcv =input('gcv k');
 k_gcv = 11;
 pause;
...
subplot(2,1,1);
plot(1:n,X_I,1:n,x,'x'), %axis([1,n,0,1.2]), 
xlabel('L = I')
subplot(2,1,2);
plot(1:n,X_L,1:n,x,'x'), %axis([1,n,0,1.2]), 
xlabel('L \neq I')
pause, clf
