...
for f=freqint(1):freqint(2);
   ...
   % Qp matrix     
      ...
      if (np < nh)   % Temporal changes to test new methods. Should be only if (np < nh) 
          %if (k==1)
             v=inv(Qp+L*CI*LH)*L*CI*(UH(f,:)).';
          %else   
          %	 vtemp=v(:);
          %	 v=inv(L*CI*LH+ 1e-6*eye(np))*(L*CI*(UH(f,:)).'- 0.1*Qp*vtemp);
			 %end	
      else    
      	QpI=diag(1./diag(Qp));
      ...
      if (f>10|k<5)
         ...
         figure(10);
         subplot(340+2*k-1);plot(log(diag(Qp)+eps));text=sprintf('iter k=%d, freq %d',k,f);title(text);
         subplot(340+2*k);plot(log(abs(v)));text=sprintf('iter k=%d, freq %d',k,f);title(text);
