...
% 	Family='Coiflet', Nfilt=3;
	...
	tres=3.0 %treshold
	tipo='MAD';
	Nwindows=16; % Can be greater than size of data.
	ncut=16*512 % Maximum index to be procesed in data.

	inmin=1;
	inmax=512*Nwindows; % Max index to be processed.
	FREQ='y'; % Data with noise due to power line.
	GRAPHS='y';  % Graphics of Wavelet coefficient series
	GRAPHW='y';  % Graphics of Wavelet Coefficients
	GRAPHWIND='y';
	GRAPHDATA='n';
	FILT='n';    % Passband in time series.
	GAN='y';

	gex=60;cgex=(10^(gex/20));
	gey=60;cgey=(10^(gey/20));
...
%-------------------------------------------------     
	load copa15.dat;datos=copa15;clear copa15;
...
%	plot(wind),figure(gcf)
	 ...
	 for i=1:1%Nwindows,
	  ...
	  exh(a:b) = fmtfilt2(ex(a:b),LS,QMF,option,tres,'y','y','ex','y');
	  eyh(a:b) = fmtfilt2(ey(a:b),LS,QMF,option,tres,GRAPHS,GRAPHW,'ey','y');
	  hxh(a:b) = fmtfilt2(hx(a:b),LS,QMF,option,tres,GRAPHS,GRAPHW,'hx','y');
	  hyh(a:b) = fmtfilt2(hy(a:b),LS,QMF,option,tres,GRAPHS,GRAPHW,'hy','y');
	  hzh(a:b) = fmtfilt2(hz(a:b),LS,QMF,option,tres,GRAPHS,GRAPHW,'hz','y');
	 end,
	else
