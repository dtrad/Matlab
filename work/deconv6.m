...
[x_I,rho,niter]=pcgnr(A,d,eye(nt),tol,zeros(nt,1), ...
			itercg);
...
for iter=1:iter_ext
  ...
  [x_L,rho,niter,Jd(iter)]=pcgnr(A,d,LI,tol,zeros(nt,1),itercg);  
%  [x_L,rho,niter,Jd(iter)]=wpcgnr2(A,d,LI,tol,zeros(nt,1),itercg);
  Jm(iter)=x_L'*inv(LI')*inv(LI)*x_L;
...
subplot(312);plot(xsol);
mytext=sprintf('(c) Left Preconditioned: %d iterations\n',iter);
mytext='b';
title(mytext);
