...
v(1)=2500;t0(1)=0.4;cr(1)=0.5;
v(2)=3300;t0(2)=0.8;cr(2)=(1-cr(1)^2)*0.5;%cr(2)=0;
...
for rr=1:nr;
   for hh=1:nh
      ...
      xx(floor(tt),hh)=(xx(floor(tt),hh)+tt2*cr(rr));
		xx(floor(tt),hh)=xx(floor(tt),hh).*avo;
      xx(ceil(tt),hh)=(xx(ceil(tt),hh)+tt1*cr(rr));
      xx(ceil(tt),hh)=xx(ceil(tt),hh).*avo;
	end;
end;
tt=1:nt;
for rr=1:nr;
...
h=(0:nh-1)*dh;t=(0:nt-1)*dt;ntlim=ceil(nt/2);
...
if options=='y' save c:\daniel\thesis\xxc.mat xxc dh dt h_near;end;
