...
% WIGB  WIGB(a,scal,x,z,amx) to plot seismic data in wiggle format.
%       'a' is input matrix.
...
trmx= max(abs(a));%if trmx==inf trmx=ones(size(a(1,:)));end
if (nargin <= 4); amx=max(trmx)/1.0;  end;
...
for i=1:nx,
  ...
  if trmx(i) ~= 0;    % skip the zero traces
	...
	% be careful at the ends
      ...
      elseif ~isempty(zadd), 		a0=0; z0=zadd(1);
         else a0=0; z0=1.00;
		end;
	...
	line( 'Color',[0.8 0.8 0.8],'EraseMode','background',  ...
