function [xsol]=reguradon(A,b_bar,x,L,e,neig,niter,eps1,eps2)
...
% dominate.
% 
% [A,b_bar,x] = shaw(32); Hanke s example
% 
[FF,DD,VV]=mapC2R(A,b_bar,x);
A=real(FF);
b_bar=real(DD);
x=real(VV);
LL=diag([real(diag(L));real(diag(L))]);
L=real(LL);
clear FF;
clear DD;
clear VV;
...
b = b_bar ;
figure,
subplot(211);plot(real(x));title('real(model) estimated with wtcgls');
subplot(212);plot(imag(x));title('imag(model) estimated with wtcgls');
%b_bar=zeros(size(b));
...
figure
subplot(2,2,1); surf(abs(X_tikh)), title('Tikhonov solutions'), axis('ij')
subplot(2,2,2); surf(log10(abs(F_tikh))), axis('ij')
                title('Tikh filter factors, log scale')
subplot(2,2,3); surf(abs(X_lsqr(:,1:17))), title('LSQR solutions'), axis('ij')
subplot(2,2,4); surf(abs(log10(F_lsqr(:,1:17)))), axis('ij')
...
display('Norms for: Tikh_l Tikh_gcv TSVD_L TSVD_GCV');
%II=input('According to these results which method for prior inf: 1,2,3,4: ');
xaxis=1:length(x_tikh_l);
figure,
subplot(221);plot(xaxis,real(x_tikh_l),xaxis,real(x),'+');
title(sprintf('Tikhonov with lambda=%e\n',lambda_l));
subplot(222);plot(xaxis,real(x_tikh_gcv),xaxis,real(x),'+');
title(sprintf('Tikhonov with GVC k=%d\n',lambda_gcv));
subplot(223);plot(xaxis,real(x_tsvd_l),xaxis,real(x),'+');
title(sprintf('Final solution with TSVD and %d eigenvectors\n',k_l));
subplot(224);plot(xaxis,real(x_tsvd_gcv),xaxis,real(x),'+');
title(sprintf('tsvd with GCV k=%d \n',k_l));

%if (II==1) xp=x_tikh_l;
%elseif (II==2) xp=x_tikh_gcv;
%elseif (II==3) xp=x_tsvd_gcv;
%elseif (II==4) xp=x_tsvd_gcv;
%end;
...
for iter=1:niter
 ...
 for i=[1,2,3,4]
  ...
  title('SVD: Right singular vectors V(:,i)'),
 end
end;
...
for i=[n,n-1,n-2,n-3]
  ...
  xlabel(['i = ',num2str(i)]), I = I + 1;
  title('GSVD: Right generalized singular vectors XX(:,i)')
end
figure,
...
if (k_tsvd > 25)
  k_tsvd =input('Input the desired number of eigenvectors for SVD');
end
if (k_tgsvd > 20)
  k_tgsvd =input('Input the desired number of eigenvectors for GSVD');
end
%k_tsvd = 7; k_tgsvd = 6;
...
figure
subplot(2,1,1);
plot(1:n,real(X_I),1:n,real(x),'x'), %axis([1,n,-1.2,1.2]), 
xlabel('L = I')
%axis1=axis;
subplot(2,1,2);
plot(1:n,real(X_L),1:n,real(x),'x'), %axis([1,n,-1.2,1.2]), xlabel('L \neq I')
%axis(axis1);
xsol=X_L(:,k_tgsvd); 

figure,
subplot(211);plot(real(x));title('Initial model');
subplot(212);plot(real(xsol));
