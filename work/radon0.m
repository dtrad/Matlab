function [v,dr,p]=radon0(d,h,np,vmin,vmax,dt,method,vtrue,eps2,step,rmethod)
...
% Example
% >>load dandata.mat   % dandata.dat contains a matrix x, offset h,time t
% >>[nt,nh]=size(x);
% >>h=0:nh-1;h=h*10;
...
switch method
 case 1
 display('Conjugate gradients Square with Tikhonov');
 case 2
...
[V,DR]=radonf1(DH,w,h,p,method,vtrue,rmethod);
V=zeropadm(V,nt/2);
DR=zeropadm(DR,nt/2);
VD=duplic(V);
DD=duplic(DR);
