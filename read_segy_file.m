...
if two_bytes(13) == 1 
   if strcmpi(param.format,'header')  | strcmpi(param.format,'ibm')
      ...
      warning(['Data apparently stored as 32-bit IBM floating point numbers; ' ...
...
elseif two_bytes(13) == 5
   if strcmpi(param.format,'header')  | strcmpi(param.format,'ieee-be')
      ...
      warning(['Data apparently stored as 32-bit IEEE big-endian floating point numbers; ' ...
         'data sample format code = ',num2str(two_bytes(13))]);
...
for jj=1:nheader
   if get.bytes{jj} == 2
      ...
      indices2=[indices2;strt];
   else
      strt=(get.first{jj}-1)/4+1;
      if fix(strt) ~= strt
         error(['First byte for header ',get.mnem{jj}, ' is wrong'])
      end
      indices4=[indices4;strt];
   end
end
