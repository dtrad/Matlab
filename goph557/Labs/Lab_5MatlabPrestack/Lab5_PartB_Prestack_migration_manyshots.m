...
for is=1:nshots
    ...
    figure,imagesc(t,x,shotrecord,CLIM);title('Full shot to be migrated');
    figure,imagesc(t,x,directwave,CLIM);title('direct wave to be migrated');
    figure,imagesc(t,x,data(:,:,is),CLIM);title('One shot to be migrated');    
end
...
for is=1:nshots
    ...
    figure;imagesc(tmig,xmig,shotmig,CLIM2);title('Migration of one shot gather with time Kirchhoff method');
    xlabel('midpoint');ylabel('time')
    migkir=migkir+shotmig;
end
figure,imagesc(tmig,xmig,migkir,CLIM2);
...
for is=1:nshots
    ...
    figure;
    imagesc(z,x,shotmigPSPI_cc,CLIM3);title('One shot PSPI with CC imaging condition');
    xlabel('midpoint');ylabel('Depth(m)');prepfig;
    migPSPI=migPSPI+shotmigPSPI_cc;
end
figure;imagesc(z,x,migPSPI,CLIM3);
title('Migration of all shots with time Kirchhoff method');
xlabel('midpoint');ylabel('time');
%%