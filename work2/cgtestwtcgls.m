...
[x,rho,eta,nit]=mywtcgls2(A,Wm,Wd,b,100,tol,1);
%x=x(:).*Wm(:); % testing going back to true model but should not do. 
nit
cg1=rho(nit)
figure(1);
subplot(311);plot(log(rho));figure(gcf);title('log(residuals)');
subplot(312);plot(tm,m,tm,x,'.');figure(gcf);axis([0 100 -10 10]);
subplot(313);plot(t,b,t,A*x,'.');figure(gcf);
figure(2);plot(tm,m,tm,x,tm,1*(m-x));axis([0 100 -10 10])

[x,rho,eta,nit]=mywtcgls3(A,Wm,Wd,b,100,tol,1);
...
subplot(312);plot(tm,m,tm,x,'.');figure(gcf);axis([0 100 -10 10]);
subplot(313);plot(t,b,t,A*x,'.');figure(gcf);
figure(4);plot(tm,m,tm,x,tm,1*(m-x));axis([0 100 -10 10])
...
%display('cond of A');
%conda;
xcgsq=conjugateGradientsSquare(A'*A,A'*b,x,nit);
figure(4);
%subplot(311);plot(log(rho));figure(gcf);title('log(residuals)');
subplot(312);plot(tm,m,tm,xcgsq,'.');figure(gcf);axis([0 100 -10 10]);
subplot(313);plot(tm,A'*b,tm,A'*A*xcgsq,'.');figure(gcf);
%figure(4);plot(tm,m,tm,x,tm,1*(m-x));axis([0 100 -10 10])
